...
/// (spec #31). Returns after the app state has already been updated.
Future<void> showTaskForm(
  ...
  return showModalBottomSheet(
    ...
    builder: (_) => _TaskFormSheet(appState: appState, existing: existing),
  );
}
...
class _TaskFormSheet extends StatefulWidget {
  ...
  const _TaskFormSheet({required this.appState, this.existing});
...
class _TaskFormSheetState extends State<_TaskFormSheet> {
  ...
  Future<void> _save() async {
    ...
    if (title.isEmpty) {
      setState(() => _error = 'Give this task a title.');
      return;
    }
    if (_reminderEnabled && _dueAt == null) {
      setState(() => _error = 'Pick a due date and time for the reminder.');
      return;
    }
    if (_reminderEnabled &&
        _dueAt!.subtract(Duration(minutes: _leadMinutes)).isBefore(DateTime.now())) {
      setState(() => _error = 'That reminder time has already passed — pick a later time.');
      return;
    }
    ...
    if (widget.existing == null) {
      ...
      await widget.appState.updateTask(widget.existing!.id, (t) => t.copyWith(
            title: title,
            description: _descCtrl.text.trim(),
            dueAt: _dueAt,
