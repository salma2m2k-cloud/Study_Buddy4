...
Future<void> showClassForm(
  ...
  return showModalBottomSheet(
    ...
    builder: (_) => _ClassFormSheet(appState: appState, existing: existing),
  );
}
...
class _ClassFormSheet extends StatefulWidget {
  ...
  const _ClassFormSheet({required this.appState, this.existing});
...
class _ClassFormSheetState extends State<_ClassFormSheet> {
  ...
  void initState() {
    ...
    _weekday = c?.weekday ?? Weekday.fromDateTimeWeekday(DateTime.now().weekday);
    _start = c != null
        ? TimeOfDay(hour: c.startHour, minute: c.startMinute)
        : const TimeOfDay(hour: 9, minute: 0);
    _end = c != null
        ? TimeOfDay(hour: c.endHour, minute: c.endMinute)
    ...
    if (!_leadOptions.contains(_leadMinutes)) _leadMinutes = 15;
  }
  ...
  Future<void> _save() async {
    ...
    if (name.isEmpty) {
      setState(() => _error = 'Give this class a name.');
      return;
    }
    ...
    if (widget.existing == null) {
      ...
      await widget.appState.updateClass(widget.existing!.id, (c) => c.copyWith(
            name: name,
            subject: _subjectCtrl.text.trim(),
            teacher: _teacherCtrl.text.trim(),
            weekday: _weekday,
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(hintText: 'Class name, e.g. Calculus II'),
              ),
              ...
              TextField(
                ...
                decoration: const InputDecoration(hintText: 'Location or link'),
              ),
              const SizedBox(height: 16),
              ...
              SwitchListTile(
                ...
                subtitle: const Text('Real notification before class starts'),
                ...
                Wrap(
                  ...
                  children: _leadOptions.map((m) {
                    ...
                    return ChoiceChip(
                      ...
                      selected: selected,
                      onSelected: (_) => setState(() => _leadMinutes = m),
                    );
                  }).toList(),
                ),
              ],
